 ...
 * @date 2020/5/8
 */
object NumPairsDivisibleBy60_1010 {
  ...
  def smartMiniHashSolution(time: Array[Int]): Int = {
    val map: Array[Int] = Array.ofDim(60)
    var cp = 0
    for (value <- time) {
      cp += map(value % 60)
      map((600 - value) % 60) += 1
  ...
  def hashSolution(time: Array[Int]): Int = {
    ...
    for (value <- time) {
      ...
      val target = remainder match {
        case 0 => 0
        case _ => 60 - remainder
      }
      map.put(target, map.getOrElse(target, 0) + 1)
    }
